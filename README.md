# QuantTrader Pro (Version 1.40 Final)
...
### 6. 远程风控联动 (EA Risk Linkage) [New v1.28]
实现 Dashboard 与 EA 的双向风控交互：
*   **实时计算**: 前端基于 Equity, Margin, H1 ATR 实时计算动态爆仓价与风险等级。
*   **动态拦截**:
    *   **SAFE**: 正常运行。
    *   **WARNING**: 静默拦截顺势加仓（防止风险扩大）。
    *   **CRITICAL**: 拦截所有开仓，并配合 UI 红色闪烁警告。
*   **双向同步**: Dashboard 开关可直接控制 EA 的开仓权限，EA 每 5 秒轮询最新风控指令。
---
...
## 版本历史
*   **v1.40**: 新增: ATR 动态止盈功能 - 支持保底+动态自适应止盈，低波动快速落袋，高波动自动放大目标
*   **v1.30**: 新增: 资金曲线图表支持时间范围切换；活跃持仓列表添加入场 RSI 显示
*   **v1.29**: 优化: ProcessSellLogic 允许在近距离挂单补仓，解除强制推远限制
*   **v1.28**: 新增: EA 远程风控联动，支持前端 Dashboard 动态计算爆仓价并拦截 EA 开仓
*   **v1.27**: 修复: 交易明细上报格式错误 (422 Unprocessable Entity)

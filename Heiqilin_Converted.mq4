...
bool     Global_ShowPanel = true;                                                      // 面板显示开关
string   PanelToggleButtonName = "PanelToggleButton";                                  // 显隐按钮名称
////////////////////////////////////////////////////////////////////////
...
extern string SECTION_DANGEROUS = "=== 危险时段控制 ===";
extern bool   Use_Dangerous_Hours = false;            // 危险时段开关
extern string DangerousHours       = "3,5,7,14,18";   // 危险时段设置（服务器时间）

//+------------------------------------------------------------------+
bool IsDangerousHour()
{
   if(!Use_Dangerous_Hours || DangerousHours == "") return false;
   int hour = TimeHour(TimeCurrent());
   string arr[];
   int count = StringSplit(DangerousHours, ',', arr);
   for(int i = 0; i < count; i++) {
      if(StringToInteger(arr[i]) == hour) return true;
   }
   return false;
}
               ...
               if(Zi_160_bo)
                 {
                  ...
                  if(!IsDangerousHour() && (On_top_of_this_price_not_Buy_order == 0.0 || (Zi_168_bo && Zi_9_in >= 1 && Zi_26_do < On_top_of_this_price_not_Buy_order) || Zi_9_in == 0 || !(Zi_170_bo)))
                    {
                     Zi_171_in = 0 ;
                     Zi_172_in = Magic ;
                     Zi_173_da = 0 ;
               ...
               if(Zi_177_bo)
                 {
                  ...
                  if(!IsDangerousHour() && (On_under_of_this_price_not_Sell_order == 0.0 || (Zi_185_bo && Zi_10_in >= 1 && Zi_26_do > On_under_of_this_price_not_Sell_order) || Zi_10_in == 0 || !(Zi_187_bo)))
                    {
                     Zi_188_in = 1 ;
                     Zi_189_in = Magic ;
                     Zi_190_da = 0 ;
   ...
   if(Mu_0_in == 1)
     {
      if((Mu_3_st == "tubiao2" || Mu_3_st == "tubiao1"))
        ...
        {
         if(Mu_3_st == PanelToggleButtonName)
           {
            Global_ShowPanel = !Global_ShowPanel;
            // 删除所有面板对象，以便重新生成
            string prefix1 = Zong_17_st_D0;
            for(int i = ObjectsTotal(0, -1, -1) - 1; i >= 0; i--)
              {
               string name = ObjectName(0, i, -1, -1);
               if(StringFind(name, prefix1, 0) >= 0) ObjectDelete(0, name);
              }
            lizong_20();
            ChartRedraw(0);
            return;
...
//----- -----
   // 创建切换按钮
   if(ObjectFind(0, PanelToggleButtonName) < 0)
     {
      ObjectCreate(0, PanelToggleButtonName, OBJ_BUTTON, 0, 0, 0);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_CORNER, 1);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_XDISTANCE, 308);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_YDISTANCE, 20);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_XSIZE, 100);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_YSIZE, 25);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_FONTSIZE, 9);
      ObjectSetString(0, PanelToggleButtonName, OBJPROP_FONT, "Microsoft YaHei");
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_BGCOLOR, C'60,60,60');
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_COLOR, White);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_BORDER_COLOR, DimGray);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_BACK, false);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_HIDDEN, false);
      ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_ZORDER, 10);
     }
   ObjectSetString(0, PanelToggleButtonName, OBJPROP_TEXT, (Global_ShowPanel ? "隐藏面板" : "显示面板"));
   ObjectSetInteger(0, PanelToggleButtonName, OBJPROP_STATE, false);

   if(!Global_ShowPanel) return;

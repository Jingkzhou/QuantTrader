...
void ReportAccountStatus() {
   ...
   for(int i=0; i<total; i++) {
      if(OrderSelect(i, SELECT_BY_POS) && OrderType() <= OP_SELLSTOP) {
         ...
         string item = StringFormat("{\"ticket\":%d,\"symbol\":\"%s\",\"side\":\"%s\",\"lots\":%.2f,\"open_price\":%.5f,\"open_time\":%d,\"profit\":%.2f,\"swap\":%.2f,\"commission\":%.2f,\"mae\":%.2f,\"mfe\":%.2f,\"open_rsi\":%.2f}",
                                    ...
                                    GlobalVariableGet(StringFormat("%sRSI_%d", GV_PREFIX, OrderTicket())));
